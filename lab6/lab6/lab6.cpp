...
// задание
void task(unsigned long long ip, unsigned long long mask, char* ip_, char* mask_)
{
...
int main(int argc, char* argv[])
{
	...
	ip = CharToLong(ip_); // переводим симвльный адресс в формат unsigned long

	if (!CheckAddress(mask_))
	{
	...
	task(ip, mask, ip_, mask_); // функция отвечающая за выполнение самого задания
